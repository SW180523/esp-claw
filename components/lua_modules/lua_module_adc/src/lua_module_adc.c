...
#define LUA_MODULE_ADC_METATABLE "adc.channel"
#define LUA_MODULE_ADC_DEFAULT_ATTEN ADC_ATTEN_DB_12

typedef enum {
    LUA_MODULE_ADC_CALI_NONE = 0,
    LUA_MODULE_ADC_CALI_CURVE,
    LUA_MODULE_ADC_CALI_LINE,
} lua_module_adc_cali_scheme_t;

typedef struct {
    adc_oneshot_unit_handle_t unit;
    adc_cali_handle_t cali;
    lua_module_adc_cali_scheme_t cali_scheme;
    adc_unit_t unit_id;
...
static esp_err_t lua_module_adc_acquire_unit(adc_unit_t unit_id,
...
static void lua_module_adc_release_unit(adc_unit_t unit_id)
{
    if (unit_id >= SOC_ADC_PERIPH_NUM) {
        return;
    }
...
static esp_err_t lua_module_adc_try_calibrate(lua_module_adc_ud_t *ud)
{
    ud->cali = NULL;
    ud->cali_scheme = LUA_MODULE_ADC_CALI_NONE;
...
#if ADC_CALI_SCHEME_CURVE_FITTING_SUPPORTED
    {
        adc_cali_curve_fitting_config_t cfg = {
            .unit_id = ud->unit_id,
            .chan = ud->channel,
            .atten = LUA_MODULE_ADC_DEFAULT_ATTEN,
            .bitwidth = ADC_BITWIDTH_DEFAULT,
        };
        if (adc_cali_create_scheme_curve_fitting(&cfg, &ud->cali) == ESP_OK) {
            ud->cali_scheme = LUA_MODULE_ADC_CALI_CURVE;
            return ESP_OK;
        }
    }
#endif
...
#if ADC_CALI_SCHEME_LINE_FITTING_SUPPORTED
    {
        ...
        if (adc_cali_create_scheme_line_fitting(&cfg, &ud->cali) == ESP_OK) {
            ud->cali_scheme = LUA_MODULE_ADC_CALI_LINE;
            return ESP_OK;
        }
    }
#endif
...
static void lua_module_adc_release_cali(lua_module_adc_ud_t *ud)
{
    if (ud->cali == NULL) {
        return;
    }
...
#if ADC_CALI_SCHEME_CURVE_FITTING_SUPPORTED
    case LUA_MODULE_ADC_CALI_CURVE:
        adc_cali_delete_scheme_curve_fitting(ud->cali);
        break;
#endif
#if ADC_CALI_SCHEME_LINE_FITTING_SUPPORTED
    case LUA_MODULE_ADC_CALI_LINE:
...
static lua_module_adc_ud_t *lua_module_adc_get_ud(lua_State *L, int idx)
{
    lua_module_adc_ud_t *ud = (lua_module_adc_ud_t *)luaL_checkudata(
        L, idx, LUA_MODULE_ADC_METATABLE);
    if (!ud || !ud->unit) {
...
static int lua_module_adc_new(lua_State *L)
{
    ...
    err = lua_module_adc_acquire_unit(unit_id, &unit);
    if (err != ESP_OK || !unit) {
    ...
    if (err != ESP_OK) {
        lua_module_adc_release_unit(unit_id);
        return luaL_error(L, "adc config channel failed: %s", esp_err_to_name(err));
    }

    lua_module_adc_ud_t *ud = (lua_module_adc_ud_t *)lua_newuserdata(
        L, sizeof(*ud));
    ud->unit = unit;
    ud->cali = NULL;
    ud->cali_scheme = LUA_MODULE_ADC_CALI_NONE;
    ...
    ud->channel = channel;
    ud->gpio_num = gpio;
    luaL_getmetatable(L, LUA_MODULE_ADC_METATABLE);
    lua_setmetatable(L, -2);

    err = lua_module_adc_try_calibrate(ud);
    if (err != ESP_OK) {
        return luaL_error(L,
...
static int lua_module_adc_read(lua_State *L)
{
...
static int lua_module_adc_gc(lua_State *L)
{
    lua_module_adc_ud_t *ud = (lua_module_adc_ud_t *)luaL_testudata(
        L, 1, LUA_MODULE_ADC_METATABLE);
    if (ud) {
        lua_module_adc_release_cali(ud);
        if (ud->unit) {
            lua_module_adc_release_unit(ud->unit_id);
            ud->unit = NULL;
        }
    }
    return 0;
...
static int lua_module_adc_close(lua_State *L)
{
    lua_module_adc_ud_t *ud = (lua_module_adc_ud_t *)luaL_checkudata(
        L, 1, LUA_MODULE_ADC_METATABLE);
    lua_module_adc_release_cali(ud);
...
{
    if (luaL_newmetatable(L, LUA_MODULE_ADC_METATABLE)) {
        lua_pushcfunction(L, lua_module_adc_gc);
    ...
    lua_newtable(L);
    lua_pushcfunction(L, lua_module_adc_new);
